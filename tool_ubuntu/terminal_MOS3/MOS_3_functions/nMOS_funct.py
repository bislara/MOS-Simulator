...
def func(Vgb, Shi_s, Vfb, NA, ND, Phi_t, q, Es, Cox, No, Po, Vcb):
    try:
        p = Vfb + Shi_s + ((sqrt(2*q*Es))/(Cox)) * (sqrt(Po*Phi_t*(e**(-Shi_s/Phi_t)-1) + (
            NA-ND)*Shi_s + No*e**(-Vcb/Phi_t)*Phi_t*(e**(Shi_s/Phi_t)-1))) - Vgb
        return p
...
def derivFunc(Shi_s, Vgb, Vfb, NA, ND, Phi_t, q, Es, Cox, No, Po, Vcb):
    ...
    t = Symbol('t')
    f = Vfb + t + (sqrt(2*q*Es)/Cox) * (sqrt(Po*Phi_t*(e**(-t/Phi_t)-1) +
                                             (NA-ND)*t + No*e**(-Vcb/Phi_t)*Phi_t*(e**(t/Phi_t)-1))) - Vgb
...
def charge_funct(NA, Phi_t, Es, q, Shi_s, Phi_F, ND, Po, No, Vcb):
    ...
    Qc = -(sqrt(2*q*Es))*(sqrt(Po*Phi_t*(e**(-Shi_s/Phi_t)-1)+(NA-ND)
                               * Shi_s+No*e**(-Vcb/Phi_t)*Phi_t*(e**(Shi_s/Phi_t)-1)))
    return Qc
...
def deriv_funct(Shi_s, Qc, NA, Phi_t, Es, q, Phi_F, Vgb, Vfb, ND, Cox, No, Po, Vcb):
    ...
    t = Symbol('t')
    V = Vfb + t + (sqrt(2*q*Es)/Cox) * (sqrt(Po*Phi_t*(e**(-t/Phi_t)-1) +
                                             (NA-ND)*t + No*e**(-Vcb/Phi_t)*Phi_t*(e**(t/Phi_t)-1)))
    deriv = Derivative(V, t)
    k = deriv.doit()

    Q = -(sqrt(2*q*Es))*(sqrt(Po*Phi_t*(e**(-t/Phi_t)-1) +
                              (NA-ND)*t+No*e**(-Vcb/Phi_t)*Phi_t*(e**(t/Phi_t)-1)))
    der = Derivative(Q, t)
    d = der.doit()
