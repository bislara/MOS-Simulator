...
def newtonRaphson(w, l, mu, Vgs, Vfb, Vds, Cox, gm, Phi_t, Phi_F, s0, Po, No, NA, ND):
    ...
    while abs(err) >= 0.001:
        count = count+1
        try:
            err = (func(w, l, mu, Vgs, Vfb, Vds, Cox, gm, Phi_t, Phi_F, s0, Po, No, NA, ND)) / \
                (derivFunc(w, l, mu, Vgs, Vfb, Vds, Cox,
                           gm, Phi_t, Phi_F, s0, Po, No, NA, ND))
            s0 = s0 - err
...
def func2(w, l, mu, Vgs, Vfb, Vds, Cox, gm, Phi_t, Phi_F, sL, Po, No, NA, ND):
    try:
        p = Vgs-Vfb-((sqrt(2*q*Es))/(Cox))*(sqrt(Po*Phi_t*(e**(-sL/Phi_t)-1) + (NA-ND)
                             * sL + No*e**(-Vds/Phi_t)*Phi_t*(e**(sL/Phi_t)-1)))-sL
...
def derivFunc2(w, l, mu, Vgs, Vfb, Vds, Cox, gm, Phi_t, Phi_F, sL, Po, No, NA, ND):
    ...
    t = Symbol('t')
    f = Vgs-Vfb-((sqrt(2*q*Es))/(Cox))*(sqrt(Po*Phi_t*(e**(-t/Phi_t)-1) + (NA-ND)
                         * t + No*e**(-Vds/Phi_t)*Phi_t*(e**(t/Phi_t)-1)))-t
    #f=Vgs-Vfb-gm*(sqrt( t+Phi_t*e**((t-2*Phi_F-Vds)/Phi_t) ))-t

    deriv = Derivative(f, t)

    # this puts the value of t as Shi_s in the derivative of the main function
    k = deriv.doit().subs({t: sL})
...
def newtonRaphson2(w, l, mu, Vgs, Vfb, Vds, Cox, gm, Phi_t, Phi_F, sL, Po, No, NA, ND):
    ...
    while abs(err) >= 0.001:
        count = count+1
        try:
            err = (func2(w, l, mu, Vgs, Vfb, Vds, Cox, gm, Phi_t, Phi_F, sL, Po, No, NA, ND)) / \
                (derivFunc2(w, l, mu, Vgs, Vfb, Vds, Cox,
                            gm, Phi_t, Phi_F, sL, Po, No, NA, ND))
            sL = sL - err
...
def calculate_Id(w, l, mu, Vgs, Vfb, Vds, Cox, gm, Phi_t, Phi_F, x0, Po, No, NA, ND):
    ...
    Shi_s0 = newtonRaphson(w, l, mu, Vgs, Vfb, Vds, Cox,
                           gm, Phi_t, Phi_F, s0, Po, No, NA, ND)
    Shi_sL = newtonRaphson2(w, l, mu, Vgs, Vfb, Vds, Cox,
                            gm, Phi_t, Phi_F, sL, Po, No, NA, ND)

    Id1 = (w/l)*mu*Cox*((Vgs-Vfb)*(Shi_sL-Shi_s0)-0.5*(Shi_sL**2 -
                                                       Shi_s0**2)-(2.0/3)*gm*(Shi_sL**(3.0/2)-Shi_s0**(3.0/2)))
    Id2 = (w/l)*mu*Cox*(Phi_t*(Shi_sL-Shi_s0) +
                        Phi_t*gm*(Shi_sL**0.5-Shi_s0**0.5))
    Id = Id1+Id2
