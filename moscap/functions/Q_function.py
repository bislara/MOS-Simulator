...
def func(Vgb, Shi_s,Vfb,NA,ND,Phi_t,q,Es,Cox,No,Po):	    
	try:
      	  p=-(sqrt(2*q*Es)) *(sqrt( Po*Phi_t*( e**(-Shi_s/Phi_t )-1) +( NA-ND  )*Shi_s + No*Phi_t*( e**(Shi_s/Phi_t )-1) )  ) -Vgb
	  print("p is ",p)	 
	  return p
...
def derivFunc( Shi_s,Vgb,Vfb,NA,ND,Phi_t,q,Es,Cox,No,Po ):
    ...
    t= Symbol('t')    
    f=-(sqrt(2*q*Es))*(sqrt( Po*Phi_t*( e**(-t/Phi_t )-1) +( NA-ND  )*t + No*Phi_t*( e**(t/Phi_t )-1) )  ) -Vgb
